...
- [~] **Phase 4 — Visual polish** — in progress.
  - **Done:** brand recolor to the gh logo (violet/magenta, see Phase 6 table); `PrimaryButton` already supports primary/secondary/ghost variants; new themed **`StatusPill`** (icon + label, tone-based) adopted in `TicketCard` + `ticket-details` — removed their hardcoded light-mode status hex (the dark-mode-broken pills) and the color-only a11y issue there.
  - **Remaining:** single type scale as a shared source (tokens has a color-bound `typography` preset to retire); roll `StatusPill` into remaining status surfaces; standardize a `Card` primitive + themed input/bottom-sheet; tasteful reanimated entrances on more screens; sub-44pt touch-target pass.
