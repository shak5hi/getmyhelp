...
- [x] **Phase 4 — Visual polish** — **COMPLETE (2026-06-25).**
  - **Brand recolor to the gh logo** (violet/magenta, see Phase 6 table) — one edit to `constants/themes.ts` reskinned the whole app in both modes.
  - `PrimaryButton` supports primary/secondary/ghost variants.
  - New themed **`StatusPill`** (icon + label, tone-based — fixes the color-only a11y issue) **rolled out across every status surface**: `TicketCard`, `ticket-details`, `visitor-history`, `resident-detail`, guard `visitor-list`. Removed all hardcoded light-mode status hex (the dark-mode-broken badges); helpers `ticketStatusTone`/`visitorStatusTone`. Resident-detail approve/reject buttons themed (were raw green/red).
  - **Deferred to Phase 5 / future polish (non-blocking):** a shared `Card` primitive + themed input/bottom-sheet; a single type scale (retire the unused color-bound `typography` preset in tokens); reanimated entrances on more screens; sub-44pt touch-target a11y pass.
